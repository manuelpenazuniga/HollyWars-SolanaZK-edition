...
const DEFAULT_MAX_PAGES = 5; // 5 pages * 100 = 500 repos; more than enough for our sampling rules
...
export class HttpGitHubClient implements GitHubClient {
  ...
  async getRepoTree(
    ...
    if (res.status === 403) {
      // 403 on a tree often means the repo is empty (no commits yet) or the
      // ref doesn't resolve. Surface a clear error rather than a vague 403.
  ...
  private async backoffFor(resetHeader: string): Promise<number> {
    ...
    // Cap at 5 minutes; add up to 2s of jitter to avoid thundering herd.
